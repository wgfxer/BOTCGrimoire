...
private suspend fun PointerInputScope.detectTransformGestures(
    ...
    forEachPointerEventUntilReleased { event ->
        android.util.Log.i("MY_INFO", "container changes size ${event.changes.size}")
 ...
 */
suspend fun AwaitPointerEventScope.forEachPointerEventUntilReleased(
    action: (PointerEvent) -> Unit,
) {
    do {
        val event = awaitPointerEvent()
        if (event.changes.fastAny { it.isConsumed } && event.changes.size < 2) {
            break
        }
        action(event)
...
fun Modifier.zoomable(
    ...
    Modifier
        ...
        .pointerInput(zoomState) {
            detectTransformGestures(
                ...
                onGesture = { centroid, pan, zoom, timeMillis ->
                    val canConsume = true//zoomState.canConsumeGesture(pan = pan, zoom = zoom)
                    android.util.Log.i("MYTAG", "canConsume $canConsume")
                    if (canConsume) {
                        scope.launch {
        ...
        .graphicsLayer {
            ...
            android.util.Log.i("MYTAG4", "translationX ${zoomState.offsetX}")
